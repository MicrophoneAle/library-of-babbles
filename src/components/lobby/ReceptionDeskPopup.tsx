...
import { RECEPTION_DESK_INTERACT_PROMPT } from "../../config/interactPrompts";
import { useInteractStore } from "../../store/interactStore";

/** Reception desk popup — opened by {@link RECEPTION_DESK_INTERACT_PROMPT}. */
export function ReceptionDeskPopup() {
  const popupId = useInteractStore((state) => state.popupId);
  const closePopup = useInteractStore((state) => state.closePopup);
  const open = popupId === RECEPTION_DESK_INTERACT_PROMPT.id;
